...
int main () {
	...
	// UNIT TEST 5
	// make sure we can push/pop a bunch of string items
	{

		Stack <std :: string> myStack;
		std :: string test[6] = {"array", "string", "test", "a", "is", "this"};
		for (int i = 0; i < 6; i++) {
			myStack.push (test[i]);
		}

		for (int i = 0; i < 6; i++) {
			QUNIT_IS_EQUAL (myStack.pop (), test[5 - i]);
		}
	...
	// UNIT TEST 5
	// make sure pop() return NULL when the stack is empty
	{
		Stack<std :: string> myStack;
	    QUNIT_IS_EQUAL (myStack.pop (), "");
	}
}
